...
void HLTStage2Info::setup(const edm::ParameterSet& pSet, TTree* HltTree) {
  ...
  l1stage2muetaAtVtx = new float[kMaxL1Stage2Mu];
  l1stage2muphiAtVtx = new float[kMaxL1Stage2Mu];
  ...
  l1stage2dimumassAtVtx = new float[kMaxL1Stage2DiMu];
  l1stage2dimuptAtVtx = new float[kMaxL1Stage2DiMu];
  l1stage2dimurapAtVtx = new float[kMaxL1Stage2DiMu];
  l1stage2dimuphiAtVtx = new float[kMaxL1Stage2DiMu];
  l1stage2dimudrAtVtx = new float[kMaxL1Stage2DiMu];
  ...
  HltTree->Branch("L1Stage2MuonEtaAtVtx",l1stage2muetaAtVtx,"L1Stage2MuonEtaAtVtx[NL1Stage2Muon]/F");
  HltTree->Branch("L1Stage2MuonPhiAtVtx",l1stage2muphiAtVtx,"L1Stage2MuonPhiAtVtx[NL1Stage2Muon]/F");
  ...
  HltTree->Branch("L1Stage2DiMuonPtAtVtx",l1stage2dimuptAtVtx,"L1Stage2DiMuonPtAtVtx[NL1Stage2DiMuon]/F");
  HltTree->Branch("L1Stage2DiMuonRapAtVtx",l1stage2dimurapAtVtx,"L1Stage2DiMuonRapAtVtx[NL1Stage2DiMuon]/F");
  HltTree->Branch("L1Stage2DiMuonPhiAtVtx",l1stage2dimuphiAtVtx,"L1Stage2DiMuonPhiAtVtx[NL1Stage2DiMuon]/F");
  HltTree->Branch("L1Stage2DiMuonInvMassAtVtx",l1stage2dimumassAtVtx,"L1Stage2DiMuonInvMassAtVtx[NL1Stage2DiMuon]/F");
  HltTree->Branch("L1Stage2DiMuondRAtVtx",l1stage2dimudr,"L1Stage2DiMuondR[NL1Stage2DiMuon]/F");
...
void HLTStage2Info::analyze(const edm::Handle<edm::TriggerResults>                 & hltresults,
  ...
  if (L1Stage2EGamma.isValid() && !L1Stage2EGamma.failedToGet()) {
    int il1stage2eg = 0;
  ...
  if (L1Stage2Muon.isValid() && !L1Stage2Muon.failedToGet()) {
    ...
    for(int iBx = L1Stage2Muon->getFirstBX(); iBx <= L1Stage2Muon->getLastBX(); ++iBx) { 
      for (l1cand muItr=L1Stage2Muon->begin(iBx); muItr!=L1Stage2Muon->end(iBx); ++muItr) {
        ...
        l1stage2muetaAtVtx[il1stage2mu] = muItr->etaAtVtx();
        l1stage2muphiAtVtx[il1stage2mu] = muItr->phiAtVtx();
        ...
        for (l1cand mu2Itr=muItr+1; mu2Itr!=L1Stage2Muon->end(iBx); ++mu2Itr) {
            ...
            TLorentzVector p1TLV = TLorentzVector(); p1TLV.SetPtEtaPhiM(muItr->pt(), muItr->eta(), muItr->phi(), 0.105658);
            TLorentzVector p2TLV = TLorentzVector(); p2TLV.SetPtEtaPhiM(mu2Itr->pt(), mu2Itr->eta(), mu2Itr->phi(), 0.105658);
            TLorentzVector p1TLVAtVtx = TLorentzVector(); p1TLV.SetPtEtaPhiM(muItr->pt(), muItr->etaAtVtx(), muItr->phiAtVtx(), 0.105658);
            TLorentzVector p2TLVAtVtx = TLorentzVector(); p2TLV.SetPtEtaPhiM(mu2Itr->pt(), mu2Itr->etaAtVtx(), mu2Itr->phiAtVtx(), 0.105658);
            reco::Particle::LorentzVector p1 = reco::Particle::LorentzVector(p1TLV.Px(),p1TLV.Py(),p1TLV.Pz(),p1TLV.E());
            reco::Particle::LorentzVector p2 = reco::Particle::LorentzVector(p2TLV.Px(),p2TLV.Py(),p2TLV.Pz(),p2TLV.E());
            ...
            reco::Particle::LorentzVector p1AtVtx = reco::Particle::LorentzVector(p1TLVAtVtx.Px(),p1TLVAtVtx.Py(),p1TLVAtVtx.Pz(),p1TLVAtVtx.E());
            reco::Particle::LorentzVector p2AtVtx = reco::Particle::LorentzVector(p2TLVAtVtx.Px(),p2TLVAtVtx.Py(),p2TLVAtVtx.Pz(),p2TLVAtVtx.E());
            reco::Particle::LorentzVector pAtVtx = p1AtVtx+p2AtVtx;
            l1stage2dimumassAtVtx[il1stage2dimu] = pAtVtx.mass();
            l1stage2dimuptAtVtx[il1stage2dimu] = pAtVtx.pt();
            l1stage2dimuphiAtVtx[il1stage2dimu] = pAtVtx.Phi();
            l1stage2dimurapAtVtx[il1stage2dimu] = pAtVtx.Rapidity();
            Double_t detaAtVtx = p1AtVtx.Eta()-p2AtVtx.Eta();
            Double_t dphiAtVtx = TVector2::Phi_mpi_pi(p1AtVtx.Phi()-p2AtVtx.Phi());
            l1stage2dimudrAtVtx[il1stage2dimu] = TMath::Sqrt( detaAtVtx*detaAtVtx+dphiAtVtx*dphiAtVtx );
            il1stage2dimu++;
            jl1stage2mu++;
        }
        il1stage2mu++;
  ...
  if (L1GOMR.isValid() && !L1GOMR.failedToGet() && !getL1InfoFromEventSetup_) {
    ...
    if (L1TEvtCnt==0){
      ...
      if(l1GtMenu.isValid() && !l1GtMenu.failedToGet()) {
