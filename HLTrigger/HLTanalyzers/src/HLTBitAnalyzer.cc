...
HLTBitAnalyzer::HLTBitAnalyzer(edm::ParameterSet const& conf)  :
  ...
  hlt_stage1_analysis_(conf, consumesCollector(), *this) {
  ...
  if (_UseL1Stage2) {
    ...
    hlt_stage1_analysis_.setup(conf, HltTree);
  }    
...
void HLTBitAnalyzer::beginRun(edm::Run const& run, edm::EventSetup const& es)
{
  if(_UseL1Stage2) {
    ...
    hlt_stage1_analysis_.beginRun(run, es);
  }
}
